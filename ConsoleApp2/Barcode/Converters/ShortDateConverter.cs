...
{
    internal class ShortDateConverter : BaseTypeConverter
    {
        ...
        public override byte[] ConvertFrom(object value)
        {
            ...
            if (dateTime < new DateTime(2000, 1, 1) || dateTime > new DateTime(2179, 6, 6))
                throw new ArgumentException("Значение даты должно быть больше 01.01.2000 и меньше 06.06.2179");
            int days = (dateTime - new DateTime(2000, 1, 1)).Days;
            return new byte[2]
            {
        (byte) ((days & 65280) >> 8),
        ...
        public override object ConvertTo(Type type, byte[] value, int startIndex, int length)
        {
            ...
            return (object)new DateTime(2000, 1, 1).AddDays((double)((int)value[startIndex] << 8 | (int)value[startIndex + 1]));
        }
    }
}
